...
### 2026-09-12 — Consolidate activity-cache display coverage

**Context:** Five near-identical display-limit tests counted stub fragment children, making optional feed metadata unnecessarily expensive to add. Two stale-cache failure fixtures also left `Date.now` mocked after cleanup.
**What happened:** Architecture decision: share the cached-event fixture and use table-driven display/failure cases; retain the legacy DOM stub to avoid unrelated fixture migration. Replaced five display-limit copies with cases for 1, 10, 11, 25, and 50 events, fresh ages of 0/1000ms, and tied/spaced timestamps. Preserved network and non-JSON refresh-failure coverage; added explicit first-event link ordering, unchanged-cache checks, semantic `.activity-item` selection, and full fixture cleanup. No application code, dependencies, scheduler settings, or proposal state changed.
**Outcome:** Removed 369 net test-file lines. Baseline isolated Linux `compound-local-green.py fabian20ro.github.io`: 175 tests passed, lint and formatting passed. After refactor: 57 focused tests and 177 full-gate tests passed. In-memory mutation probes confirmed optional status siblings pass, while limits of 9/11, reversed cache order, and replacement of cached content on refresh failure fail the intended cases. Live lifecycle inventory was idle; branch already at the five-commit review cap before edits.
**Insight:** A test should count the domain elements whose limit it specifies, not incidental wrapper children. The shared fixture also exposed previously missing clock/session cleanup.
**Promoted to Lessons Learned:** Yes — semantic activity selection and fixture cleanup.

### 2026-09-15 — Repair live activity freshness and user-visible status

**Context:** Reported stale activity and an unstyled bare “20 hours” timestamp. Source reproduction confirmed dropped timestamps on language change, no ordinary tab-resume refresh, mixed-language error UI, unconditional browser `module.exports`, and modern workflow badges navigating to repository roots. Work performed under the host-maintained Compound reservation; no merge, push, proposal state change, or deployment.
**What happened:** One activity-state owner now keeps events, last-success time, status, and in-flight request. Replaced the obsolete once-per-session whole-page reload guard with lazy-init-preserving pageshow/visibility refresh and a one-minute visible tick controlled by the ten-minute TTL. Relative-time repaint preserves DOM identity. Requests single-flight, abort after fifteen seconds, retain saved results on failure, distinguish successful empty results, throttle automatic failures, and honor GitHub rate-limit headers. Added all status translations across seven dictionaries and themed timestamp styling; language switches render complete state. Guarded CommonJS exports, corrected actual badge Actions destinations and the English arrow, bumped CSS/JS cache versions.
**Verification:** New isolated actual-script lifecycle tests first failed on the browser `module` error; the completed seven-case suite covers repeated resumes, exact TTL, concurrent triggers, focus-preserving repaint, hidden/lazy behavior, timeout abort, rate-limit failure, manual recovery, locale transitions, malformed cache/API data, and actual project badge URLs. `npm run check`: lint, formatting, and 192 tests passed. Legacy cache fixtures now isolate module state. Corrected assertions that explicitly expected naked timestamps, repository-root badge destinations, API-error coercion to cached empty success, or shared test state. Replaced an exported-primitive assignment pseudo-test with rendered behavior; awaited previously concurrent contract fixtures.
**Insight:** Passing unit tests were insufficient: several tests enshrined the regression, and an export-assignment test never observed the real internal state. Keep independent lifecycle acceptance and browser review separate from generated implementation tests. Browser/deployed acceptance status is recorded by the coordinating verifier; a local commit is not a deployment.
